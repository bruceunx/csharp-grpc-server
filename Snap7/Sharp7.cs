...
namespace Sharp7
{
    ...
    class MsgSocket
    {
            ...
            if (!Connected)
            {
                ...
                try
                {
                    try
                    {
                        ...
                        Task.WaitAny(
                            Task.Run(async () => await AsConnect(Host, Port.ToString(), cts))
                        );
                    }
                    catch
                    {
        ...
        {
            get { return (TCPSocket != null) && _Connected; }
        }
    ...
    class MsgSocket
    {
        ...
        public bool Connected
        {
            get { return (TCPSocket != null) && (TCPSocket.Connected); }
        }
...
#if WINDOWS_UWP || NETFX_CORE
                    if (IntPtr.Size == 4)
                        Items[Count].pData = (IntPtr)(
                            handle.AddrOfPinnedObject().ToInt32() + Offset * Marshal.SizeOf<T>()
                        );
                    else
                        Items[Count].pData = (IntPtr)(
                            handle.AddrOfPinnedObject().ToInt64() + Offset * Marshal.SizeOf<T>()
                        );
#else
                    if (IntPtr.Size == 4)
                        Items[Count].pData = (IntPtr)(
    ...
    public class S7Client
    {
        ...
        { // 7 bytes
            ...
            0x00,
            0x00,
            0x1f, // Telegram Length (Data Size + 31 or 35)
            0x02,
            0xf0,
            0x80 // COTP (see above for info)
        };
        ...
        byte[] S7_PN =
        {
            ...
            0x01,
            0x00,
            0x1e // PDU Length Requested = HI-LO Here Default 480 bytes
        };
        ...
        { // 31-35 bytes
            ...
            0x00, // PDU Reference
            0x00,
            0x0e, // Parameters Length
            0x00,
            0x00, // Data Length = Size(bytes) + 4
            ...
            0x04, // Transport size
            0x00,
            0x00, // Data Length * 8 (if not bit or timer or counter)
        };
        ...
        byte[] S7_MWR_HEADER =
        {
            ...
            0x00, // PDU Reference
            0x00,
            0x0e, // Parameters Length (idx 13)
            0x00,
            0x00, // Data Length = Size(bytes) + 4 (idx 15)
        ...
        public int ReadMultiVars(S7DataItem[] Items, int ItemsCount)
        {
            ...
            for (int c = 0; c < ItemsCount; c++)
            {
                ...
                if (S7ItemRead[0] == 0xff)
                {
                    ...
                    if (
                        (S7ItemRead[1] != TS_ResOctet)
                        && (S7ItemRead[1] != TS_ResReal)
                        && (S7ItemRead[1] != TS_ResBit)
                    )
                        ItemSize = ItemSize >> 3;
        ...
        public int WriteMultiVars(S7DataItem[] Items, int ItemsCount)
        {
            ...
            for (int c = 0; c < ItemsCount; c++)
            {
                ...
                ;
                if (
                    (Items[c].WordLen == S7Consts.S7WLTimer)
                    || (Items[c].WordLen == S7Consts.S7WLCounter)
                )
